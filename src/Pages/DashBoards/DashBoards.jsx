    ...
    return (
        <div className="grid grid-cols-4 gap-4 mx-20 my-10">
                ...
                <div className="bg-yellow-400 p-6 rounded-md">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} method="dialog">
                        ...
                        <div className="form-control mt-6 p-0">
                            <button className="btn bg-white  hover:bg-yellow-400 hover:text-white text-yellow-500 border-1 border-yellow-400 font-bold">Submit</button>
                        </div>
                    </form>
                </div>
            ...
            <div>
                ...
                <div>
                    {
                        tasks.map(task => <div key={task._id} className="bg-lime-300 mb-4 p-4 rounded-md space-y-1">
                            <h1 className="font-bold">{task?.title}</h1>
                            <p>{task?.description}</p>
                            <p>{task?.date}</p>
                            <button className="px-2 py-1 bg-lime-600 font-semibold rounded-full text-white">{task?.priority}</button>
                        </div>)
                    }
                </div>
            </div>
